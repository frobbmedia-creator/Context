 ...
 * v0.3.0 — Context Score + magical watch output
 */
...
import { computeContextScore, formatScoreLine } from './score.js';
...
function inferLanguage(filePath) {
  ...
  const map = {
    ...
    mjs: 'JavaScript', cjs: 'JavaScript', css: 'CSS', scss: 'CSS', html: 'HTML',
    md: 'Markdown', markdown: 'Markdown', json: 'JSON', yml: 'YAML', yaml: 'YAML',
    py: 'Python', rs: 'Rust', go: 'Go', java: 'Java', c: 'C', h: 'C/C++ Header',
    cpp: 'C++', hpp: 'C++ Header', rb: 'Ruby', php: 'PHP', swift: 'Swift',
    kt: 'Kotlin', sql: 'SQL', sh: 'Shell', bash: 'Shell', zsh: 'Shell',
...
export async function collectSources(root, { extraIgnore = [], useGit = true } = {}) {
  ...
  const entries = await fg('**/*', { cwd: root, onlyFiles: true, dot: false, followSymbolicLinks: false, suppressErrors: true, absolute: false });
  ...
  for (const rel of entries) {
    ...
    if (!binary) {
      ...
      } catch { warnings.push('read-error'); }
    }
    const language = inferLanguage(rel);
    const priority = gitPrio.get(norm) ?? (stat.mtimeMs > Date.now() - 86400000 ? 80 : 20);
    sources.push({ path: norm, name: path.basename(rel), size: stat.size, mtime: stat.mtimeMs, language, binary, priority, text, warnings });
  }
  return sources;
}

export async function analyzeAndPack(sources, options) {
  const { budget = 128000, reserved = 4000, model = 'claude', compression = 'structural', format = 'xml', prompt = '', includeOmitted = true } = options;
  const available = Math.max(0, budget - reserved);
  const records = [];
  for (const src of sources) {
    if (src.binary) {
      records.push({ ...src, tokens: 0, fullTokens: 0, content: '', contentHash: '', included: false, ignoreReason: 'binary-file' });
      continue;
    }
    ...
    records.push({
      path: src.path, name: src.name, size: src.size, mtime: src.mtime, language: src.language, binary: false,
      priority: src.priority, tokens, fullTokens, content: compressed, contentHash,
      hashScope: src.warnings.includes('sampled-large-file') ? 'sampled-text' : 'full-text',
      warnings: src.warnings, tokenizer: tokenizerLabel(model), compressor: 'Context structural compressor v2',
      compression, included: false
    });
  }
  const ranked = [...records].filter(r => !r.binary).sort((a, b) => b.priority - a.priority || a.tokens - b.tokens);
  let used = 0;
  const includeSet = new Set();
  for (const r of ranked) {
    if (used + r.tokens <= available) { used += r.tokens; includeSet.add(r.path); }
  }
  for (const r of records) r.included = includeSet.has(r.path);
  ...
  const stats = { total: records.length, included: included.length, omitted: omitted.length, tokens, saved, available };
  const score = computeContextScore(stats, records);
  return { content, stats, score, records };
}
...
export async function watchWorkspace(root, options = {}) {
    ...
    try {
      ...
      if (options.out === 'clipboard') {
        try {
          ...
          const scoreLine = result.score ? formatScoreLine(result.score) : '';
          console.error(`[${new Date().toLocaleTimeString()}] ✓ ${result.stats.included} files · ${result.stats.tokens} tok · saved ${result.stats.saved} → clipboard`);
          if (scoreLine) console.error(`   ${scoreLine}`);
        } catch (e) {
  ...
  const schedule = () => { clearTimeout(timer); timer = setTimeout(run, options.debounceMs || 400); };
